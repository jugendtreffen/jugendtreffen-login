...
export const createParticipant: MutationResolvers['createParticipant'] = async ({input}) => {
    const {email} = input
    const result = await db.participant.create({
        data: input,
    })
    logger.info(`Created participant with email ${email} and name ${input.name}`)
    await sendRegistrationConfirmation({to: email, name: input.name})
    return result
}

export const updateParticipant: MutationResolvers['updateParticipant'] = ({id, input}) => {
...
export const deleteParticipant: MutationResolvers['deleteParticipant'] = ({id}) => {
