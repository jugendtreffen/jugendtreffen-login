...
const InfoField = ({ label, value }) => {
  return (
    <div
      className="bg-gradient-to-br from-gray-500/10 to-gray-600/10 border border-gray-500/20 rounded-xl p-3 inline-flex gap-3">
...
export const Empty = () =>
  <div>
    ...
    <Alert id={"empty"} message={"Du bist nirgends angemeldet"} dismissible={false} />
  </div>;
...
export const Failure = ({
  ...
  <div>
    <h1 className="mb-2">Deine Anmeldungen</h1>
    <Alert type={"error"} id={error?.name} message={error?.message} dismissible={false} />
  </div>
);
        ...
        {participations.map((item) => {
          return <li key={item.id}>
            <Card className="flex flex-col gap-2">
              ...
              <InfoField label="Unterkunft" value={item.accommodation ? "Bei Jugendtreffen" : "Private Unterkunft"} />
              <InfoField label="Essen" value={item.foodChoice} />

              <div className="flex items-center gap-4">
                <div
                  className="flex-1 bg-gray-700/30 border border-gray-600/50 rounded-xl p-4 text-center hover:bg-gray-700/40 transition-all duration-300 hover:-translate-y-0.5">
                ...
                <div
                  className="flex items-center justify-center w-10 h-10 bg-gradient-to-r from-secondary to-accent-dark rounded-full flex-shrink-0">
